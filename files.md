...
| `src/job-validator.js` | Shared validation primitives - exports validateByHead(url), validateByContent(url, opts), DEFAULT_EXPIRED_KEYWORDS. Used by both `validate-jobs.js` and `tests/validate-sobis-solutions-jobs.js`. |
...
| `tests/unit/index.test.js` | Unit tests for index.js - parseAnofmJobs, mapToJobModel, transformJobsForSOLR |
...
| `tests/e2e/scraper.test.js` | E2E tests - full pipeline with real ANOFM API, ANAF, and SOLR |
...
- Full workflow: validate company (ANAF+Peviitor) → scrape ANOFM → transform → upsert SOLR → generate docs/jobs.md
